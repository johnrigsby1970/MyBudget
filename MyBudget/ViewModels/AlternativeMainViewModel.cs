...
public class AlternativeMainViewModel : ViewModelBase
{
    ...
    public AlternativeMainViewModel()
    {
    ...
    public Bill? EditingBillClone { get => _editingBillClone; set => SetProperty(ref _editingBillClone, value); }
    public BudgetBucket? EditingBucketClone { get => _editingBucketClone; set => SetProperty(ref _editingBucketClone, value); }
    public Account? EditingAccountClone { get => _editingAccountClone; set => SetProperty(ref _editingAccountClone, value); }
    public AdHocTransaction? EditingAdHocTransactionClone { get => _editingAdHocTransactionClone; set => SetProperty(ref _editingAdHocTransactionClone, value); }
    ...
    public ICommand DeleteAdHocTransactionCommand => new RelayCommand(t => DeleteAdHocTransaction(t as AdHocTransaction));
    ...
    private void LoadPeriodBills()
    {
        ...
        if (addedAny) pBills = _budgetService.GetPeriodBills(CurrentPeriodDate).ToList();
    ...
    private void Item_PropertyChanged(object? sender, System.ComponentModel.PropertyChangedEventArgs e)
    {
        ...
        if (sender is Paycheck p) _budgetService.UpsertPaycheck(p);
    ...
    private void AddBill() { var bill = new Bill { Name = "New Bill", ExpectedAmount = 0, DueDay = 1, IsActive = true }; _budgetService.UpsertBill(bill); LoadData(); CalculateProjections(); }
    private void EditBill() { if (SelectedBill != null) { EditingBillClone = new Bill { Id = SelectedBill.Id, Name = SelectedBill.Name, ExpectedAmount = SelectedBill.ExpectedAmount, Frequency = SelectedBill.Frequency, DueDay = SelectedBill.DueDay, AccountId = SelectedBill.AccountId, ToAccountId = SelectedBill.ToAccountId, NextDueDate = SelectedBill.NextDueDate, Category = SelectedBill.Category, IsActive = SelectedBill.IsActive }; IsEditingBill = true; } }
    private void SaveBill() { if (EditingBillClone != null && SelectedBill != null) { UpdateBillFromClone(SelectedBill, EditingBillClone); _budgetService.UpsertBill(SelectedBill); IsEditingBill = false; EditingBillClone = null; CalculateProjections(); } }
    private void UpdateBillFromClone(Bill target, Bill clone) { target.Name = clone.Name; target.ExpectedAmount = clone.ExpectedAmount; target.Frequency = clone.Frequency; target.DueDay = clone.DueDay; target.AccountId = clone.AccountId; target.ToAccountId = clone.ToAccountId; target.NextDueDate = clone.NextDueDate; target.Category = clone.Category; target.IsActive = clone.IsActive; }
    private void CancelBill() { IsEditingBill = false; EditingBillClone = null; }
    private void DeletePeriodBill(PeriodBill? pb) { if (pb != null) { _budgetService.DeletePeriodBill(pb.Id); LoadPeriodBills(); CalculateProjections(); } }
    public ICommand DeleteBillCommand => new RelayCommand(b => DeleteBill(b as Bill));
    private void DeleteBill(Bill? b) { if (b != null) { _budgetService.DeleteBill(b.Id); LoadData(); CalculateProjections(); } }
    
    private void AddBucket() { var bucket = new BudgetBucket { Name = "New Bucket", ExpectedAmount = 0 }; _budgetService.UpsertBucket(bucket); LoadData(); CalculateProjections(); }
    private void EditBucket() { if (SelectedBucket != null) { EditingBucketClone = new BudgetBucket { Id = SelectedBucket.Id, Name = SelectedBucket.Name, ExpectedAmount = SelectedBucket.ExpectedAmount, AccountId = SelectedBucket.AccountId }; IsEditingBucket = true; } }
    private void SaveBucket() { if (EditingBucketClone != null && SelectedBucket != null) { UpdateBucketFromClone(SelectedBucket, EditingBucketClone); _budgetService.UpsertBucket(SelectedBucket); IsEditingBucket = false; EditingBucketClone = null; CalculateProjections(); } }
    private void UpdateBucketFromClone(BudgetBucket target, BudgetBucket clone) { target.Name = clone.Name; target.ExpectedAmount = clone.ExpectedAmount; target.AccountId = clone.AccountId; }
    private void CancelBucket() { IsEditingBucket = false; EditingBucketClone = null; }
    private void DeletePeriodBucket(PeriodBucket? pb) { if (pb != null) { _budgetService.DeletePeriodBucket(pb.Id); LoadPeriodBills(); CalculateProjections(); } }
    public ICommand DeleteBucketCommand => new RelayCommand(b => DeleteBucket(b as BudgetBucket));
    private void DeleteBucket(BudgetBucket? b) { if (b != null) { _budgetService.DeleteBucket(b.Id); LoadData(); CalculateProjections(); } }

    private void AddAdHocTransaction() { var t = new AdHocTransaction { Description = "New Ad-Hoc", Amount = 0, Date = DateTime.Today, PeriodDate = CurrentPeriodDate }; _budgetService.UpsertAdHocTransaction(t); LoadPeriodBills(); CalculateProjections(); SelectedAdHocTransaction = CurrentPeriodAdHocTransactions.FirstOrDefault(x => x.Description == "New Ad-Hoc" && x.Amount == 0); EditAdHocTransaction(); }
    private void EditAdHocTransaction() { if (SelectedAdHocTransaction != null) { EditingAdHocTransactionClone = new AdHocTransaction { Id = SelectedAdHocTransaction.Id, Description = SelectedAdHocTransaction.Description, Amount = SelectedAdHocTransaction.Amount, Date = SelectedAdHocTransaction.Date, AccountId = SelectedAdHocTransaction.AccountId, ToAccountId = SelectedAdHocTransaction.ToAccountId, BucketId = SelectedAdHocTransaction.BucketId, PeriodDate = SelectedAdHocTransaction.PeriodDate, IsPrincipalOnly = SelectedAdHocTransaction.IsPrincipalOnly, IsRebalance = SelectedAdHocTransaction.IsRebalance, PaycheckId = SelectedAdHocTransaction.PaycheckId, PaycheckOccurrenceDate = SelectedAdHocTransaction.PaycheckOccurrenceDate }; IsEditingAdHocTransaction = true; } }
    private void SaveAdHocTransaction() { if (EditingAdHocTransactionClone != null && SelectedAdHocTransaction != null) { UpdateAdHocFromClone(SelectedAdHocTransaction, EditingAdHocTransactionClone); _budgetService.UpsertAdHocTransaction(SelectedAdHocTransaction); IsEditingAdHocTransaction = false; EditingAdHocTransactionClone = null; LoadPeriodBills(); CalculateProjections(); } }
    private void UpdateAdHocFromClone(AdHocTransaction target, AdHocTransaction clone) { target.Description = clone.Description; target.Amount = clone.Amount; target.Date = clone.Date; target.AccountId = clone.AccountId; target.ToAccountId = clone.ToAccountId; target.BucketId = clone.BucketId; target.PeriodDate = clone.PeriodDate; target.IsPrincipalOnly = clone.IsPrincipalOnly; target.IsRebalance = clone.IsRebalance; target.PaycheckId = clone.PaycheckId; target.PaycheckOccurrenceDate = clone.PaycheckOccurrenceDate; }
    ...
    private void DeleteAdHocTransaction(AdHocTransaction? t) { if (t != null) { _budgetService.DeleteAdHocTransaction(t.Id); LoadPeriodBills(); CalculateProjections(); } }

    private void AddPaycheck() { var p = new Paycheck { Name = "New Paycheck", ExpectedAmount = 0, StartDate = DateTime.Today, Frequency = Frequency.BiWeekly }; _budgetService.UpsertPaycheck(p); LoadData(); CalculateProjections(); }
    public ICommand DeletePaycheckCommand => new RelayCommand(p => DeletePaycheck(p as Paycheck));
    private void DeletePaycheck(Paycheck? p) { if (p != null) { /* _budgetService.DeletePaycheck(p.Id); // Missing in service? */ LoadData(); CalculateProjections(); } }
    private void AddAccount() { var a = new Account { Name = "New Account", Type = AccountType.Checking, Balance = 0, BalanceAsOf = DateTime.Today, IncludeInTotal = true }; _budgetService.UpsertAccount(a); LoadData(); CalculateProjections(); }
    private void EditAccount() { if (SelectedAccount != null) { EditingAccountClone = new Account { Id = SelectedAccount.Id, Name = SelectedAccount.Name, BankName = SelectedAccount.BankName, Balance = SelectedAccount.Balance, BalanceAsOf = SelectedAccount.BalanceAsOf, AnnualGrowthRate = SelectedAccount.AnnualGrowthRate, IncludeInTotal = SelectedAccount.IncludeInTotal, Type = SelectedAccount.Type }; IsEditingAccount = true; } }
    private void SaveAccount() { if (EditingAccountClone != null && SelectedAccount != null) { UpdateAccountFromClone(SelectedAccount, EditingAccountClone); _budgetService.UpsertAccount(SelectedAccount); IsEditingAccount = false; EditingAccountClone = null; CalculateProjections(); } }
    private void UpdateAccountFromClone(Account target, Account clone) { target.Name = clone.Name; target.BankName = clone.BankName; target.Balance = clone.Balance; target.BalanceAsOf = clone.BalanceAsOf; target.AnnualGrowthRate = clone.AnnualGrowthRate; target.IncludeInTotal = clone.IncludeInTotal; target.Type = clone.Type; }
    private void CancelAccount() { IsEditingAccount = false; EditingAccountClone = null; }
    private void DeleteAccount(Account? a) { if (a != null) { _budgetService.DeleteAccount(a.Id); LoadData(); CalculateProjections(); } }

    private void ShowAmortization(Account? account) { /* ... */ }
    private void ShowAbout() { var about = new AboutWindow(); about.Owner = Application.Current.MainWindow; about.ShowDialog(); }
