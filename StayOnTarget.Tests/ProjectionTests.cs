...
namespace StayOnTarget.Tests
{
    ...
    public class ProjectionTests
    {
        ...
        public void TestBillAccountRobustness_UsesAccountId()
        {
            ...
            var accounts = new List<Account>
            {
                new Account { Id = 1, Name = "Checking", Balance = 1000, IncludeInTotal = true, BalanceAsOf = today.AddMonths(-1) },
                new Account { Id = 2, Name = "Savings", Balance = 5000, IncludeInTotal = false, BalanceAsOf = today.AddMonths(-1) }
            };
        ...
        public void TestCreditCard_UserScenario_InterestAfterGracePeriod()
        {
            ...
            // startOfMonth + 1 month - 1 day: Transaction 14.40. Balance 14.40.
            // startOfMonth + 1 month statement: interest 0 because startOfMonth balance was 0 (grace period).
            //               balance remains 14.40. PaidInFull = false.
            // startOfMonth + 2 months statement: interest SHOULD accrue on 14.40 (ADB = 14.40).
            
            var marchStatement = results.FirstOrDefault(r => r.Description.Contains("Credit Card Interest") && r.TransactionDate == startOfMonth.AddMonths(1));
            ...
            var aprilStatement = results.FirstOrDefault(r => r.Description.Contains("Credit Card Interest") && r.TransactionDate == startOfMonth.AddMonths(2));
            Assert.IsNotNull(aprilStatement, "April statement (4/5) should exist");
            
            // Expected interest for startOfMonth + 2 months:
            // 14.40 * (0.1774 / 365) * 31 days = 0.217... (roughly $0.22)
            // But wait, if grace was active in startOfMonth + 1 month (because startOfMonth balance was 0), interest on startOfMonth + 2 months would also be 0
            // unless the startOfMonth + 1 month balance was not paid in full by the due date.
            ...
            var novStatement = results.FirstOrDefault(r => r.Description.Contains("Credit Card Interest") && r.TransactionDate == startOfMonth.AddMonths(9));
            Assert.IsNotNull(novStatement, "November statement (11/5) should exist");
            Assert.IsTrue(Math.Abs(novStatement.AccountBalances["CreditCard"]) > Math.Abs(octStatement.AccountBalances["CreditCard"]), "Balance should continue to increase month over month");
        }
        ...
        public void TestPeriodNet_CalculatedCorrected()
        {
            ...
            var accounts = new List<Account>
            {
                new Account { Id = 1, Name = "Checking", Balance = 1000, IncludeInTotal = true, BalanceAsOf = startOfMonth }
            };
            var paychecks = new List<Paycheck>
            {
                new Paycheck { Id = 1, Name = "Pay1", ExpectedAmount = 2000, Frequency = Frequency.BiWeekly, StartDate = startOfMonth, AccountId = 1 },
                new Paycheck { Id = 2, Name = "Pay2", ExpectedAmount = 2000, Frequency = Frequency.BiWeekly, StartDate = startOfMonth.AddDays(14), AccountId = 1 }
            };
            var bills = new List<Bill>
            {
            ...
            // Period 2: startOfMonth + 14 onwards. Events: Pay1 (2000), Pay2 (2000), Bill1 (-500). Total = 3500.
            // Since they are on the same day, Pay: Pay1 should be the first item and have the PeriodNet.
            var pay1SecondOccurrence = results.FirstOrDefault(r => r.Description == "Expected Pay: Pay1" && r.TransactionDate == startOfMonth.AddDays(14));
            Assert.IsNotNull(pay1SecondOccurrence);
            Assert.AreEqual(3500m, pay1SecondOccurrence.PeriodNet);

            var pay2Entry = results.FirstOrDefault(r => r.Description == "Expected Pay: Pay2" && r.TransactionDate == startOfMonth.AddDays(14));
        ...
        public void TestTransactionInterest_OverridesProjectedInterest()
        {
            ...
            var accounts = new List<Account>
            {
                ...
                { 
                    ...
                    {
                        InterestRate = 6.0m,
                        PaymentDate = startOfMonth.AddDays(14)
                    }
                }
            };

            // Transaction on the same date as projected interest
            ...
            var transactions = new List<Transaction>
            {
                new Transaction
                {
                    ...
                    Description = "Actual Interest",
                    Amount = -950,
                    TransactionDate = startOfMonth.AddDays(14),
                    AccountId = 1,
                    IsRebalance = true
                }
            };
            ...
            var interestEntries = results.Where(r => r.TransactionDate == startOfMonth.AddDays(14)).ToList();
        ...
        public void TestBucketReduction_TransactionReducesProjectedBucket()
        {
            ...
            var accounts = new List<Account>
            {
                new Account { Id = 1, Name = "Checking", Balance = 1000, IncludeInTotal = true, BalanceAsOf = baseDate }
            };
            var paychecks = new List<Paycheck>
            {
                new Paycheck { Id = 1, Name = "Pay1", ExpectedAmount = 2000, Frequency = Frequency.BiWeekly, StartDate = baseDate, AccountId = 1 }
            };
